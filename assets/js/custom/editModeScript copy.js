...
//         sub: ["Corporate", "Agency", "Startup"]
//     },
//     {
//         title: "Gym",
//         icon: "ri-heart-pulse-fill",
//         sub: ["Fitness", "Yoga", "Workout"]
//     },
//     {
//         title: "Restaurant",
...
//     script.innerHTML = `
//         window.categories = ${JSON.stringify(window.categoryData || [])};
//     `;
    ...
    function uploadImgData(file, originalEl) {
        ...
        $.ajax({
            ...
            success: function () {
                ...
                const previewPath = `/media/projects/${clientName}/${clientProjectName}/${originalSrc}`;
                ...
                if ($(originalEl).hasClass('site-logo-img')) {
                    ...
                    container.find('.generated-logo').hide();
                    $(originalEl).show();
                }
...
$(document).off('click', '#categoryDropdownButton');

$(document).off('click', '#categoryDropdownButton');

$(document).on('click', '#categoryDropdownButton', function (e) {
...
$(document).off('click', '#sectionDropdownButton');

$(document).on('click', '#sectionDropdownButton', function (e) {

    e.preventDefault();
    e.stopPropagation();

    $('#sectionDropdownMenu').toggle();
...
$(document).on('click', '.category-parent', function (e) {
    ...
    const collapseId = $(this).attr('data-collapse');

    const $sub = $('#' + collapseId);

    $('.category-sub').not($sub).hide();
    ...
    `);
        ...
        // Section Filter
        $(document).on('click', '#sectionDropdownMenu a', function (e) {
            e.preventDefault();
            selectedSection = $(this).data('value');
            $('#sectionDropdownButton').html(
    `${$(this).text()} <span class="caret"></span>`
);

$('#sectionDropdownMenu').hide();
            filterComponents();
        });
        ...
        //         const componentSection = ($(this).attr('subsection') || '').toLowerCase();

        //         const categoryMatch =
        //             (selectedCategory.toLowerCase() === 'all') ||
        //             (componentCategory.includes(selectedCategory.toLowerCase()));

        //         const sectionMatch =
        //             (selectedSection.toLowerCase() === 'allsections') ||
        //             (componentSection === selectedSection.toLowerCase());
...
$(document).on('click', function (e) {
    ...
    if (
        !$(e.target).closest('#categoryDropdownButton').length &&
        !$(e.target).closest('#categoryDropdownMenu').length
    ) {
    ...
    function handleSaveSection() {
        ...
        $('#dynamicModal .section-checkbox:checked').each(function () {
            selectedIds.unshift($(this).val());
        });
